      ...
      <Card className="border-primary/20 bg-primary/5">
        <CardContent className="py-4">
          <div className="flex items-center justify-between gap-4 flex-wrap">
            <span className="font-medium text-foreground">Multiplicar receta</span>
            <div className="flex items-center gap-2">
              ...
              <Input
                ...
                className="w-20 text-center font-mono font-semibold shrink-0"
                step={0.25}
                min={0.25}
              />
              <Button
          ...
          <div className="space-y-2">
            {(() => {
              ...
              return gruposKeys.map((grupoNombre, grupoIndex) => (
                <div key={grupoNombre}>
                  {tieneGrupos && (
                    <div className={`text-xs font-semibold uppercase tracking-wider text-muted-foreground ${grupoIndex > 0 ? 'mt-4' : ''} mb-2`}>
                      {grupoNombre}
                    </div>
                  )}
          ...
          <ol className="space-y-3">
            {receta.instrucciones.map((instruccion, index) => (
              <li key={index} className="flex gap-3">
                ...
                <span className="text-foreground leading-relaxed text-[15px]">{instruccion}</span>
              </li>
            ))}
          </ol>
        </CardContent>
